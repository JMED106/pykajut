...
def parser(config_file, arguments=None, description=None, usage=None, parameters=True):
    ...
    try:
        ...
        log.warning("The configuration file '%s' is missing" % config_file)
    ...
    for group in options:
        if parameters and group in ("Parameters", "parameters", "options", "Options"):
            prmts = {'dummy': 0}
        gr = pars.add_argument_group(group)
        args = options[group]
        for key in options[group]:
            flags = key.split()
            if prmts and group in ("Parameters", "parameters", "options", "Options"):
